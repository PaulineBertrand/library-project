...
module.exports = function findCoverImage(req, res, next) {
    ...
    const titlePart = removeAccents(req.body.title).toLowerCase().split(' ').join('+');
    const authorPart = removeAccents(req.body.author).toLowerCase().split(' ').join('+');
    const coverIdUrl = '/search.json?title=' + titlePart + '&author=' + authorPart;
